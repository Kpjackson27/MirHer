...
for (var i = 0, len = symbolsArr.length; i < len; i++) {
    symbolsObj[symbolsArr[i]] = i;
}
var defaults = {
    parts: 3,
    partLen: 4,
...
module.exports.referralCode = function(email, min, max) {
    var code = '';
    console.log(typeof email);
    console.log(email);
    do {
        var letters = email.split('@')[0];
        if (letters.length >= 4 && !hasBadWord(code)) { //use first 4 letter of email
            code = letters.substr(0, 4).toUpperCase();
        } else {
            code = randomLetters(4);
        }
    } while (hasBadWord(code));
    return code + randomNum(min, max);
};
// --------------------------------------------------------------------------------------------------------------------
// internal helpers
...
function randomLetters(len) {
    var letters = '';
    for (var i = 0; i < len; i++) {
        letters += lettersArr[parseInt(Math.random() * lettersArr.length, 10)];
    }
    return letters;
}

function randomNum(min, max) {
    return parseInt(Math.random() * (max - min) + min, 10).toString();
}
